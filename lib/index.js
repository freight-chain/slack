...
function slackin({
  ...
  if (
    ...
    throw new Error(
      "Both `recaptcha-secret` and `recaptcha-sitekey` must be defined to enable reCAPTCHA."
    );
  }
  ...
  if (channels) {
    channelsFiltered = channels
      .split(",")
      .map((channel) => (channel.startsWith("#") ? channel.slice(1) : channel));
  }
  ...
  if (themeID) {
    if (themeID in themes) {
      ...
      mainLog(
        `Specified theme (${themeID}) not found, falling back to default`
      );
    }
  } else {
  ...
  const superagentDist = resolvePkg("superagent/superagent.js", {
    cwd: __dirname,
  });
  ...
  app.get("/", (req, res) => {
    ...
    return res.type("html").render("main", {
      coc,
      path: relativePath,
      name,
      org,
  ...
  app.post("/invite", json(), (req, res) => {
    ...
    if (channelsFiltered && !channelsFiltered.includes(channel)) {
      ...
      recaptcha.secret &&
      (!captchaResponse || !captchaResponse.length)
    ) {
  ...
  // iframe
  const logo = read(sysPath.join(__dirname, "/../assets/slack.svg")).toString(
    "base64"
  );
  const js = read(sysPath.join(__dirname, "/../assets/iframe.js")).toString();
  const extraCss = read(
    sysPath.join(__dirname, "/../assets/iframe-button.css")
  ).toString();
  ...
  // realtime
  sockets(srv, { path: `${relativePath}socket.io` }).on(
    "connection",
    (socket) => {
